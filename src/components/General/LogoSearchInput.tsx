...
export const LogoSearchInput: React.FC<Props> = (props) => {
   ...
   const delayedQuery = useCallback(_.debounce((q: string) => {
      handleQueryChange(q)
      setQuery(q)
   }, 100), []);
   ...
   return (
      <React.Fragment>
         ...
         <input type="text" defaultValue={query} onChange={(e) => delayedQuery(e.target.value)}
                autoFocus autoComplete="off" style={{textIndent: "20px"}}
